...
本页通过其主要功能的示例对 Dart 语言进行了简要介绍。

## Hello World

每个应用程序都需要顶级的`main()`函数，这是执行开始的地方。函数如果没有明确返回一个值，那么就有`void`返回类型。若要在控制台上显示文本，你可以使用顶级的函数`print()`：
...
即使在类型安全的Dart代码中，你也可以使用var声明大多数变量，而无需明确指定它们的类型。多亏了类型推断，这些变量的类型是由它们的初始值决定的：
